...
def local_boundary(mat, cid, k, inf, nc, tc):
    ...
    if len(cands) > 1:
        ...
        # print(mattemp)
        embedding = MDS(n_components=nc, normalized_stress=False,dissimilarity='precomputed')
...
def auto_mat(file,title, R='auto', errthre=0.1, inf=100, ep=0.1, top=6, \
    ...
    err = 1
    err1=1
    while err > errthre and err1==1:
        sigmaind2,kappaind2 = cover_to_rad(cp_lam,cid_s,cid_k,\
                            title, R='auto', inf=inf, ep=ep, \
                            sigmaind=sigmaind1,kappaind=kappaind1, top=top, option='reg', \
                                            figtitle=r'E:\流形研究\20250317代码整理\temp.png')
        err = max(abs((sigmaind1 - sigmaind)/sigmaind),abs((kappaind1-kappaind)/kappaind))
        if (sigmaind,kappaind)==(sigmaind2,kappaind2):
            err1=0
        sigmaind,kappaind = sigmaind1,kappaind1
        sigmaind1, kappaind1=sigmaind2,kappaind2
